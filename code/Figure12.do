...
// Graph Setup
global ytitle	= "Total Trade with the World (million 1957 USD)"
global xtitle   = "Year"
global treat_graph  = $treat_y -1
	   
// Total Trade with the World
twoway line trade year if country=="Cuba", lpattern(solid)  ///
	|| line trade_synth  year if country=="Cuba", lpattern(dash) ///
	xline($treat_graph)	 ///
	xlabel(1920(5)1979, labsize(small)) ///
	ylabel(0(1000)10000, labsize(small)) ///
	ytitle($ytitle, size(small)) xtitle($xtitle, size(small)) ///
	ysize(5) xsize(6)	///
	   legend(label(1 "Cuba") label(2 "Synthetic Cuba") ///
	   position(6) rows(1) size(small))  
	   
graph save "$path/figures/Figure12.gph", replace
graph export "$path/figures/Figure12.png", replace
