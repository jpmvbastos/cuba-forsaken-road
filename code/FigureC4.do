...
*|
*| Function: Generates SDID estimates for GDP fully adjusted. Produces Figure C4
*|==============================================================================
...
// Fully Adjusted Maddison GDP
//------------------------------------------------------------------------------

global pre_treatment = "full_adjustment_80av_covariate full_adjustment_84av_covariate  full_adjustment_87av_covariate full_adjustment_89av_covariate"

set seed 1234 
sdid full_adjustment id year treatment, covariates( $predictors $pre_treatment) vce(placebo) reps(100) ///
		method(sdid) graph g2_opt(ytitle("Fully Adjusted GDP per capita") ///
...
graph save g2_1991 "$path/figures/FigureC4-A", replace
graph export "$path/figures/FigureC4-A.png", as(png) replace

set seed 1234 
sdid_event full_adjustment id year treatment, covariates( $predictors $pre_treatment) vce(placebo) brep(100) placebo(all) method(sdid) 
  ...
  mat res = e(H)
		...
		ytitle(Fully Adjusted GDP per capita (Effect)) yline(0, lc(cranberry) lp(-)) /// 
...
graph save "$path/figures/FigureC4-B", replace
graph export "$path/figures/FigureC4-B.png", as(png) replace
