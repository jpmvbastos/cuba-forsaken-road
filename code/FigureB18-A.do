...
*|
*| Function: Replicates Figure B18-A
*|==============================================================================
...
cap drop _shift*
gen _shift = gdppc23 - gdppc23_demean
gen _shift_synth = gdppc23_synth_demean + _shift

// Get midpoint for intercept 
qui sum _shift_synth if country=="Cuba" & year==1934
scalar up = r(mean)

qui sum gdppc23_synth if country=="Cuba" & year==1934
scalar low = r(mean)
...
twoway (line gdppc23 year if country=="Cuba", lpattern(solid) lcolor(midblue)) ///
	|| (line _shift_synth year if country=="Cuba", lpattern(dash) lcolor(cranberry)) ///
	|| (line gdppc23_synth year if country=="Cuba", lpattern(dash) lcolor(cranberry*.5)) ///
...
graph save "$path/figures/FigureB18-A.gph", replace
graph export "$path/figures/FigureB18-A.png", replace
