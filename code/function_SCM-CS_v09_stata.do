...
* Acknowledgement: We are deeply thankful to Kristyna Brunova
* (https://ies.fsv.cuni.cz/en/staff/brunova) for improvement in our Stata
* function. Any remaining errors are our own.
*******************************************************************************
...
/* updated to be flexible */
if(`treated' == 1) {
matrix Y0 = Y[1..., (`treated'+1)...]
}
else if(`treated' == colsof(Y)) {
...
if "`etype'" == "constant" {
	...
	matrix ones = J(`nrows', 1, 1)
	matrix postp = zero\ones
	matrix gapsafter = gaps[(`T0' + 1)..., 1]
	matrix nh0 = zero\gapsafter
}

di "param is set to:" `param'
****************************************
	...
	* confidence set.
	...
	while `reject_u' == 0 {
				...
				* Create the matrices of observed outcomes under the null.
				...
				if `j' == `treated' {
					...
					if `j' < `treated' {
						...
						if `treated' == 2 {
							matrix Y0temp1 = Y0[1..., 2...] /* here there was only the first row of Y0 before */
							/* changed to Y0[1..., 2...] */
							matrix Y0 = temp,Y0temp1
						}

						/* added condition for `treated' == `nregion': */
							...
							matrix Y0temp1 = Y0[1...,1..`treated' - 2]
							matrix Y0temp2 = Y0[1..., `treated'...] /* BUT Y0 has only (`nregion'-1) columns, therefore if `treated' == `nregion', this won't work
							                                            hence else if statement above added */
						...
						if `treated' == `nregion' - 1 {
							...
							matrix Y0temp1 = Y0[1..., `treated' + 1...] /* added condition for `treated' == 1 */
							matrix Y0 = temp,Y0temp1
						
						}
						
						else {
							matrix Y0temp1 = Y0[1...,1..`treated' - 1] /* this would throw an error if `treated' == 1, hence else if statement above added */
		...
		* Create a vector with the assignment probabilities.
		...
		forvalues j = 1(1)`nregion' {
			local temp1 = v[1,`j'] /* co je tadyto v? nemysli nahodou jeden z argumentu funkce, v? */
	...
	while `reject_l' == 0 {
		...
		* Estimate the gaps for each placebo unit.
		forvalues j = 1(1)`nregion' {
			* Create the matrices of observed outcomes under the null.
			...
			else { /* this part was changed in a similar manner as the part above */
				matrix Y1 = Y[1..., `j'] + nh
				if `j' == 1 {
					matrix Y0 = Y[1...,2...]
				}
		...
		* Update the lower bound parameter
		if `reject_l' == 0 {
			...
			if `attemptl1' == 1 {
				local errorl = "The confidence set is empty for the requested class of functions."
				display "`errorl'"
				local reject_l = 1
			}
			if "`etype'" == "uniform" {
		...
		if "`etype'" == "uniform" {
			...
			if abs(`lb') > 100*abs(`param') {
				local errorl = "Upper bound was not found."
				local reject_l = 1
			}
		}
	}
...
* Upper bound
	...
	* Upper bound
	matrix upper = nh0 + `ub'*postp
	* Lower bound
	matrix lower = nh0 + `lb'*postp
...
// Return the matrix so it's accessible after the program ends 
// ADDED BY JP
