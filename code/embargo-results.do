...
allsynth us_trade /// Outcome
	...
	pvalues(rmspe) ///
	keep($path/results/us_trade-results.dta, replace)

	mat V = e(V_matrix)
	esttab mat(V) using "$path/results/v-matrix/us-trade.dta", ///
	replace mlab(none)
...
allsynth us_imports /// Outcome
	...
	unitnames(country_moxlad) resultsperiod($x_axis) ///
	pvalues(rmspe) ///
	keep($path/results/us_imports-results.dta, replace)

	mat V = e(V_matrix)
	esttab mat(V) using "$path/results/v-matrix/us-imports.dta", ///
	replace mlab(none)
...
allsynth trade /// Outcome
	...
	pvalues(rmspe)  ///
	keep($path/results/trade-results.dta, replace)
	
	mat V = e(V_matrix)
	esttab mat(V) using "$path/results/v-matrix/trade.dta", ///
	replace mlab(none)
...
save "$path/results/embargo-results.dta", replace
